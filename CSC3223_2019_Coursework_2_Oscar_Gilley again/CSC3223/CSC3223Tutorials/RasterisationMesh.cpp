...
RasterisationMesh* RasterisationMesh::CreateTriangleFromPoints(
	const std::vector<Vector3>& inVerts, const std::vector<Vector4>& inColours, int type) {
...
Maths:ScreenBoxOfTri(p0, p1, p2, topLeft, bottomRight);
	...
	for (int y = (int)topLeft.y; y < bottomRight.y; ++y) {
		for (int x = (int)topLeft.x; x < bottomRight.x; ++x) {
			...
			//if the sum of subtri areas is bigger than the area of the triangle, skip it
			if ((int)triSum > abs(triArea)) {
				continue;
			}

			float alpha = subTriArea[1] * areaRecip;
			float beta = subTriArea[1] * areaRecip;
			float gamma = subTriArea[1] * areaRecip;
...
void RasterisationMesh::RasterSpanTriangle(
	...
	//the longest edge will now always be vetices[0] and [2]
